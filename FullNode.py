...
class FullNode(full_node_pb2_grpc.FullNodeServicer):
	...
	def display_full_blockchain(self,request,context):
		print ('Receiving web request to display the blockchain')
		response = full_node_pb2.BlockchainReply(response=pickle.dumps(self.blockchain))
		return response
	...
	def show_all_existing_contracts(self,request,context):
		contracts = [pickle.dumps(x) for x in self.contract_pool.list]
		print ('Displaying all available bets to the web server!')
		response = full_node_pb2.ExistingContractReply(serialized_contracts=contracts)
		return response
	...
	def new_contract_broadcast(self, request, context):
		...
		print (contract) #AKASH ADDITION DEBUGGING DELETE THIS LATER!
		need_to_add = True
